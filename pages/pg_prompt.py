"""pg_prompt.py v17 — Development Info (Core Prompt HIDDEN)"""
...
def render():
    ...
    st.caption("Core IMRAD Intelligence v17 · 27 Modul · +99% dari v13")
    ...
    k1.metric("Versi","v17","Latest")
    k2.metric("Modul Core",str(len(CORE_MODULES)),"+5 v17")
    ...
    st.divider()
    st.subheader("🚀 Fitur yang Dikembangkan v17")

    features = [
        ("📊 Content Ratio v17","Results section: 40% tabel + 60% gambar dihitung dari file yang diunggah. Tidak ditentukan manual — sistem hitung otomatis dari naskah."),
        ("🔗 Discussion Linker v17","Discussion wajib referensi ke judul Table/Figure yang dibuat di Results. Sistem ekstrak judul tabel/gambar dari naskah dan inject ke instruksi Discussion."),
        ...
        ("📑 Citation 60 Total v17","50 sitasi base + 20% × 50 = 10 dari jurnal target = 60 sitasi total. Tag [TARGET-JOURNAL] vs [CORE-DB] eksplisit."),
        ("👥 CRediT Form v17 (M29)","Input n_authors + nama + 14 CRediT roles per penulis. First Author + Co-Authors otomatis di-label. Generate CRediT Statement untuk Prompt E."),
        ("💾 Session Persistence v17 (M28)","Streamlit Cloud akan kehilangan data saat disconnect. Tombol Simpan Sesi → unduh state.json. Pulihkan Sesi → restore 5 info + jurnal + authors."),
        ("🌐 GDrive Auto-Read v17 (M30)","Setelah download dari GDrive: otomatis parse PDF/DOCX, extract data kuantitatif, dan auto-assign sebagai naskah utama jika belum ada."),
        ("📖 GDrive In-Memory v17 (M18/M30)","File & folder GDrive dibaca LANGSUNG ke memori (BytesIO), TANPA download ke disk. Folder via HTML scrape file IDs. Confirm-token untuk file > 25MB. Diagnosa error: PRIVATE vs NOT-FOUND vs RATE-LIMIT."),
    ]
    ...
    hist=pd.DataFrame([
        ...
        {"Versi":"v17","Deskripsi":"40/60 tabel/gambar, Discussion←Results, 1950 kata, canvas baru per stage, 10 judul, Daftar Pustaka APA+DOI"},
    ])
